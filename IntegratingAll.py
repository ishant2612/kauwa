# Original don't change any thing 
...
# Did something for labels . doesn't work
...
#     def verify_query(self, query):
#     # Extract labels using ML model
#         labels = self.label_extractor.extract_labels(query)

#     # Verify using VerificationAgent
#         verification_result = self.verification_system.process_query(query)
#         boolean_result = verification_result['verification']['verdict'] == 'TRUE'
#         confidence = float(verification_result['verification']['confidence'])
#         reason = verification_result.get('verification', {}).get('reason', "Reason not available")

#         # Add to knowledge graph if verified
#         if boolean_result:
#             self._add_query_to_graph(query, verification_result, labels)

#         return {
#             "is_true": boolean_result, 
#             "confidence": confidence, 
#             "reason": reason,
#             "labels": labels
#         }

#     def _add_query_to_graph(self, query, verification_result, labels):
#         if not self.driver:
#             return
...
#             try:
#                 session.execute_write(
#                     self._create_labeled_query_nodes,
#                     query_id,
#                     query,
#                     verification_result,
#                     labels
...
#     @staticmethod
#     def _create_labeled_query_nodes(tx, query_id, query_text, verification_result, labels):
#         # Merge on query_id to ensure uniqueness
#         query = """
#         MERGE (q:Query {query_id: $query_id})
#         SET q.text = $query_text,
#             q.is_true = $is_true,
...
#         # Add labels separately to avoid conflicts
#         for label in labels:
#             label_query = """
#             MATCH (q:Query {query_id: $query_id})
#             SET q:{label}
#             """.format(label=label.replace(' ', '_'))
#             tx.run(label_query, query_id=query_id)
