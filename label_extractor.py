...
class LabelExtractor:
    def __init__(self):
        # Load the pre-trained SpaCy model once and reuse
        try:
            self.nlp = spacy.load("en_core_web_sm", disable=["parser", "ner"])  # Disable components we don't need (like parser and NER)
        except OSError:
            print("SpaCy model not found. Please download using 'python -m spacy download en_core_web_sm'")
            raise

        # Define keywords for different labels (optimized with lowercase keywords for fast lookup)
        self.label_keywords = {
            "politics": set([word.lower() for word in [
                "government", "election", "policy", "minister", "senate", "congress", "president", "democracy",
                "republic", "campaign", "politician", "democrat", "republican", "senator", "voter", "party", 
                "legislation", "candidates", "vote", "parliament", "foreign policy", "public opinion", "NGO", 
                "activist", "diplomacy", "UN", "peacekeeping", "embassy", "sanction", "political parties", 
                "referendum", "military", "policy reform", "lawmaker", "constitutional", "judiciary", 
                "Supreme Court", "political unrest", "political debate"
            ]]),
            "tech": set([word.lower() for word in [
                "technology", "software", "AI", "computer", "hardware", "samsung", "apple", "google", "microsoft",
                "amazon", "tesla", "nvidia", "intel", "facebook", "twitter", "cloud computing", "blockchain", 
                "iot", "5g", "augmented reality", "virtual reality", "cybersecurity", "data science", "machine learning", 
                "artificial intelligence", "big data", "quantum computing", "automation", "robotics", "cryptocurrency", 
                "smartphone", "app development", "wearable tech", "internet of things", "data privacy", "deep learning"
            ]]),
            "sports": set([word.lower() for word in [
                "game", "match", "tournament", "player", "team", "football", "basketball", "cricket", "baseball", 
                "soccer", "athlete", "coach", "stadium", "championship", "olympics", "world cup", "super bowl", 
                "nba", "fifa", "rugby", "hockey", "golf", "tennis", "formula 1", "swimming", "boxing", "wrestling", 
                "mma", "combat sports", "sportsmanship", "league", "fan", "sports event", "sporting goods", "esports"
            ]]),
            "business": set([word.lower() for word in [
                "business", "company", "corporation", "startup", "entrepreneur", "finance", "investment", "economics", 
                "market", "industry", "venture", "growth", "shareholders", "ceo", "management", "merger", "acquisition", 
                "revenue", "sales", "business model"
            ]]),
        }
    ...
    def extract_labels(self, query: str) -> List[str]:
        ...
        Returns:
            List[str]: List of extracted labels
        """
        query_lower = query.lower()  # Lowercase once for consistency
        labels = set()
        ...
        # SpaCy entity recognition for additional context (only keeping the most relevant entities)
        doc = self.nlp(query)

        # Extract labels from named entities (specific categories for political or business-related entities)
        entity_labels = {
            "ORG": ["business", "tech"],
            "PERSON": ["politics", "sports"],
            "GPE": ["politics", "business"],
        }

        for ent in doc.ents:
            if ent.label_ in entity_labels:
                labels.update(entity_labels[ent.label_])

        # If multiple labels, prioritize specific ones or choose the most relevant label
        if len(labels) > 1:
            # Prioritize "tech", "politics", or "sports" based on the query content
            if "tech" in labels:
                return ["tech"]
            if "politics" in labels:
                return ["politics"]
